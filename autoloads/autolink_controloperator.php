...
class MyAutoLinkOperator
{
    /*!
     Constructor
    */
    function MyAutoLinkOperator()
    {
        $this->Operators = array( 'myautolink');
    }
    ...
    */
    function namedParameterList()
    {
        ...
        return array( 'myautolink' => array(
        								'text' => array('type' => 'string', 'required' => true, 'default' => '' ),
                                        'length' => array('type' => 'string', 'required' => false, 'default' => '' ),
                                        'add' => array('type' => 'string', 'required' => false, 'default' => '' )
                                           ));
    }
    ...
    */
    function modify( $tpl, $operatorName, $operatorParameters, $rootNamespace, $currentNamespace, &$operatorValue, $namedParameters )
    {
        switch ( $operatorName )
        {
            case 'myautolink':
            {
                $operatorValue = $this->autolink( $namedParameters['text'], $namedParameters['length'], $namedParameters['add'] );
            } break;
        }
    }

    function myautolink( $text  )
    { 
	...
	function autolink($text, $limit=30, $tagfill=''){
		...
		$text = $this->autolink_do($text, 'https://',	$limit, $tagfill);
		$text = $this->autolink_do($text, 'http://',	$limit, $tagfill);
		$text = $this->autolink_do($text, 'ftp://',	$limit, $tagfill);
		$text = $this->autolink_do($text, 'www.',	$limit, $tagfill);
		return $text;
	}
